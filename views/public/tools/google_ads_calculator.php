<?php
// views/public/tools/google_ads_calculator.php - Interactive Google Ads ROI Calculator (exact ayeuburrahman.work design)
?>
    ...
    <div class="container" style="max-width: 980px; margin: 0 auto; padding: 0 20px;">
        ...
        <nav aria-label="Breadcrumb" style="margin-bottom: 24px; display: inline-flex;">
            <ol style="display: inline-flex; align-items: center; gap: 8px; padding: 6px 16px; border-radius: 6px; background: #f1f5f9; border: 1px solid #e2e8f0; font-size: 0.85rem; font-weight: 500; color: #64748b; list-style: none; margin: 0;">
                ...
                <li><i class="fa-solid fa-angle-right" style="font-size: 0.72rem; color: #94a3b8;"></i></li>
                <li style="color: #0f172a; font-weight: 600;">Google Ads ROI Calculator</li>
            </ol>
        </nav>
        ...
        <div style="text-align: center; margin-bottom: 40px;">
            ...
            <p style="font-size: 1.1rem; color: #475569; max-width: 680px; margin: 0 auto; line-height: 1.6;">
                Model the return on a Google Ads budget using your own conversion rates and customer value.
            </p>
        </div>
        ...
        <div style="max-width: 620px; margin: 0 auto 50px;">
            <div class="tool-main-card">
                <form id="googleAdsForm" onsubmit="event.preventDefault(); openGateModal();">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        ...
                        <div class="tool-form-group">
                            <label class="tool-form-label" for="average_cpc">
                                Average CPC <span style="font-size: 0.75rem; color: #64748b; font-weight: normal;">(USD)</span>
                            </label>
                            <input id="average_cpc" name="average_cpc" type="number" inputmode="decimal" min="0" step="0.01" value="2.5" class="tool-form-control">
                        </div>
                    </div>
                    <p class="tool-field-hint" style="margin-top: -12px; margin-bottom: 18px;">
                        Search CPC varies widely by keyword competition. Use the figure from your own account where you have one.
                    </p>
                ...
                <div class="tool-result-card">
                    ...
                    <div class="tool-result-header">
                        ...
                        <p class="tool-result-amount">
                            <span id="resRoas">3.00x</span> <span style="font-size: 1.1rem; font-weight: 600; opacity: 0.85;">ROAS</span>
                        </p>
                        <p style="font-size: 0.95rem; color: rgba(255,255,255,0.92); margin-top: 4px;">
                            Profit after ad spend: <strong id="resProfitAfterSpend">$4,000</strong>
                        </p>
                    </div>
                    ...
                    <div style="padding: 24px 28px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 24px;">
                            <div class="tool-stat-pill">
                                ...
                                <div class="stat-val" id="resClicks">800</div>
                            </div>
                            <div class="tool-stat-pill">
                                <div class="stat-lbl">Leads</div>
                                <div class="stat-val" id="resLeads">40</div>
                            </div>
                            <div class="tool-stat-pill">
                                <div class="stat-lbl">Customers</div>
                                <div class="stat-val" id="resCustomers">8</div>
                            </div>
                            <div class="tool-stat-pill">
                                <div class="stat-lbl">Revenue</div>
                                <div class="stat-val" id="resRevenue">$12,000</div>
                            </div>
                            <div class="tool-stat-pill">
                                <div class="stat-lbl">Gross Profit</div>
                                <div class="stat-val" id="resGrossProfit">$4,800</div>
                            </div>
                            <div class="tool-stat-pill">
                                <div class="stat-lbl">Cost Per Sale (CPA)</div>
                                <div class="stat-val" id="resCpa">$250</div>
                            </div>
                        </div>
                        ...
                        <div style="border-top: 1px solid #f1f5f9; padding-top: 20px; display: flex; flex-direction: column; gap: 14px;">
                            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                                ...
                                <a href="https://wa.me/<?= preg_replace('/[^0-9]/', '', setting('whatsapp_number', '+8801670769816')) ?>?text=Hello%20Abdullah%2C%20I%20used%20your%20Google%20Ads%20ROI%20Calculator%20and%20would%20like%20to%20discuss%20my%20numbers." target="_blank" rel="noopener" class="tool-wa-btn">
...
<div id="gateModal" class="digi-thankyou-modal-overlay">
    <div class="digi-thankyou-modal-card" style="max-width: 480px; text-align: left; padding: 28px;">
        ...
        <form action="<?= url('/contact/submit') ?>" method="POST" id="googleLeadGateForm" onsubmit="handleGateSubmit(event)">
            <?= csrf_field() ?>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                ...
                <div class="tool-form-group" style="margin-bottom: 0;">
                    ...
                    <input type="text" id="gateCity" name="city" class="tool-form-control" placeholder="New York / London">
                </div>
            </div>
...
<script>
let currentGoogleCalc = null;

function computeGoogleEstimate() {
    ...
    const drivers = [
        { label: 'Monthly Search Ad Budget', amount: '$' + budget.toLocaleString('en-US') },
        { label: 'Target CPC', amount: '$' + cpc.toFixed(2) },
    ...
    return currentGoogleCalc;
}

function openGateModal() {
    computeGoogleEstimate();
...
function renderGoogleResults() {
    const c = currentGoogleCalc || computeGoogleEstimate();
...
function handleGateSubmit(e) {
    ...
    const c = currentGoogleCalc || computeGoogleEstimate();
    const specs = `GOOGLE ADS CALCULATOR ESTIMATE: ${c.roas.toFixed(2)}x ROAS, Budget: $${c.budget}, Clicks: ${c.clicks}, Leads: ${c.leads}, Customers: ${c.customers}, Revenue: $${c.revenue}, Profit: $${c.profitAfterSpend}`;
