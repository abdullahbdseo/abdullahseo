...
export const DB = {
  ...
  updateOrder: (id, data) => {
    const idx = ordersStore.findIndex(o => o.id === parseInt(id) || o.order_number === id);
    if (idx !== -1) {
      ordersStore[idx] = { ...ordersStore[idx], ...data };
      if (data.status === "in_progress" || data.status === "completed") {
        ordersStore[idx].payment_status = "confirmed";
      }
  ...
  updateInquiry: (id, data) => {
    ...
    if (idx !== -1) {
      inquiriesStore[idx] = { ...inquiriesStore[idx], ...data };
